...
public class EncoderController {
	...
	public static interface H264DateListener{
		void h264DateCallback(byte[] data, int width, int height, long timestemp);
	}
	...
	public EncoderController(Context context,TextureView textureView){
		...
		mEncoderManager = EncoderFactory.createEncoder();
	}
	...
	private void setupCamear(SurfaceTexture surface) {
		...
		mCamera.setPreviewCallbackWithBuffer(new PreviewCallback() {
			...
			public void onPreviewFrame(byte[] data, Camera camera) {
				...
				mEncoderManager.offerEncoder(data);
		...
		if (mEncoderManager == null) {
			mEncoderManager = new EncoderSDKVersion16();
		}
		mEncoderManager.initEncoder(mWidth, mHeight, mFramerate,
				512,
				para.getPreviewFormat(), mCameraOrientation);
		mEncoderManager.setCallBack(new EncodeManager.DateCallBack() {
			@Override
			public void callback(byte[] data, MediaCodec.BufferInfo info) {
				long timestemp= info.presentationTimeUs / 1000;
				int width=mWidth;
				int height=mHeight;
				...
				if(mH264DateListener!=null){
					mH264DateListener.h264DateCallback(data, width, height, timestemp);
				}
			}
		});

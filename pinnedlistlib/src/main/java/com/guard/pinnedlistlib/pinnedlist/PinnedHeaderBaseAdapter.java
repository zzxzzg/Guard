 ...
 */
public abstract class PinnedHeaderBaseAdapter extends CompositeBaseAdapter
    ...
    public PinnedHeaderBaseAdapter(Context context, int initialCapacity) {
    ...
    public View getPinnedHeaderView(int partition, View convertView, ViewGroup parent) {
        if (hasHeader(partition)) {
            ...
            bindHeaderView(view, partition, getObject(partition));
...
//
//                listView.setHeaderPinnedAtBottom(i, listHeight - bottomHeaderHeight, false);//底部的图钉是否显示
//                maxBottomHeader = i;
//            }
//        }
...
//        }
        ...
        /**-------------------------------type 2------------------------------------------**/
        //modify by yxwang type 2
        int pos=listView.getFirstVisiblePosition();
        int partition=getPartitionForPosition(pos);
        int topHeight=listView.getPinnedHeaderHeight(partition);
        int pos2 = listView.getPositionAt(topHeight) - headerViewsCount;
        int partition2 = getPartitionForPosition(pos2);
        if(partition==partition2){
        	listView.setHeaderPinnedAtTop(partition, 0, false);
            for (int i = 0; i < size; i++) {
                if (mHeaderVisibility[i]&&i!=partition) {
                    listView.setHeaderInvisible(i, isPartitionEmpty(i));
                }
            }
        }else{
        	int fp=listView.getFirstVisiblePosition();
        	View headerview=listView.getChildAt(pos2-fp);
        	int top=headerview.getTop();
        	listView.setHeaderPinnedAtTop(partition, -topHeight+ top, false);
        	listView.setHeaderPinnedAtTop(partition2,top, false);
        }
        /**-------------------------------type 2------------------------------------------**/
    }
    int lastTopHeaderHeight=0;

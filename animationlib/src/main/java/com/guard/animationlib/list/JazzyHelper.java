...
import com.guard.animationlib.list.effects.CardsEffect;
import com.guard.animationlib.list.effects.CurlEffect;
import com.guard.animationlib.list.effects.FadeEffect;
import com.guard.animationlib.list.effects.FanEffect;
import com.guard.animationlib.list.effects.FlipEffect;
import com.guard.animationlib.list.effects.FlyEffect;
...
import com.guard.animationlib.list.effects.WaveEffect;
import com.guard.animationlib.list.effects.ZipperEffect;
...
//        setTransitionEffect(transitionEffect);
    ...
    public final void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        onScrolled(view, 0,firstVisibleItem, visibleItemCount, totalItemCount);
        notifyAdditionalOnScrollListener(view, firstVisibleItem, visibleItemCount, totalItemCount);
    }

    public final void onScrolled(ViewGroup viewGroup,int before, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        if (mIsScrolling && shouldAnimateItems) {
            ...
            while (firstVisibleItem + indexAfterFirst < mFirstVisibleItem) {
                ...
                View item = viewGroup.getChildAt(before+indexAfterFirst);
    ...
    private void doJazzinessImpl(View item, int position, int scrollDirection) {
        if(item==null || item.animate()==null){
            return;
        }

...
//	int keyy=0;
	...
	public void startEncoder(final int framerate, final int bitrate) {
		...
		mEncodeThread = new EncodeThread(framerate,bitrate);
		mEncodeThread.mFlag=true;
		mEncodeThread.start();
	}
	...
	private class EncodeThread extends Thread{
		...
		public EncodeThread(int framerate,int bitrate){
		...
		public void run() {
			try {
				mFlag = true;
				setParameters(mCameraWidth, mCameraHeight, mBitRate);
							...
							if (mDateCallBack != null) {
								...
								if (sps == null && pps == null) {
									...
									if (spsPpsBuffer.getInt() == 0x00000001) {
										...
										System.arraycopy(data, 4, sps, 0,
												sps.length);
										pps = new byte[data.length - ppsIndex];
										System.arraycopy(data, ppsIndex, pps, 0,
												pps.length);
...
//									android.util.Log.d("sss","the width="+width+"   "+height);
	...
	public void stopEncoder() {
		...
		mEncodeThread.mFlag=false;
		mEncodeThread.interrupt();
		//remove for bug 56632
//		try {
//			mEncodeThread.join();
//		} catch (InterruptedException e) {
//			// TODO Auto-generated catch block
...
//		mHeight = height ;
	...
	private static int selectColorFormat(MediaCodecInfo codecInfo,
		...
		if (VERBOSE)
		{
			Log.d(TAG, "" + capabilities.colorFormats.length);

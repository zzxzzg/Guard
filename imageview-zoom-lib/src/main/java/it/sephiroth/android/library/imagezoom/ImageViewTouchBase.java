...
public abstract class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    public List<PointF> getOverlayPoint() {
        return mOverlayPoint;
    }

    public void setOverlayPoint(List<PointF> mOverlayPoint,List<Bitmap> bitmaps) {
        this.mOverlayPoint = mOverlayPoint;
        mOverlayBitmaps=bitmaps;
    }

    public void addOverlayPointF(PointF pointf,Bitmap bitmap){
        if(mOverlayPoint==null){
            mOverlayPoint=new ArrayList<PointF>();
            mOverlayBitmaps=new ArrayList<Bitmap>();
        }
        mOverlayPoint.add(pointf);
        mOverlayBitmaps.add(bitmap);
        invalidate();
    }

    public void addOverlayPointF(PointF pointf,Bitmap bitmap,int index){
        if(mOverlayPoint==null){
            mOverlayPoint=new ArrayList<PointF>();
            mOverlayBitmaps=new ArrayList<Bitmap>();
        }
        mOverlayPoint.add(index,pointf);
        mOverlayBitmaps.add(index,bitmap);
        invalidate();
    }

    public void addOverlayPointF(List<PointF> pointfs,List<Bitmap> bitmaps){
        if(mOverlayPoint==null){
            mOverlayPoint=new ArrayList<PointF>();
            mOverlayBitmaps=new ArrayList<Bitmap>();
        }

        mOverlayPoint.addAll(pointfs);
        if(bitmaps!=null) {
            mOverlayBitmaps.addAll(bitmaps);
        }
        invalidate();
    }
    ...
    protected void onDetachedFromWindow() {
        ...
        if(mOverlayBitmaps==null){
            return;
        }
        for(Bitmap bitmap:mOverlayBitmaps){
    ...
    public List<Bitmap> getOverlayBitmaps() {
...
//    }
    ...
    private List<PointF> mOverlayPoint;
    private List<RectF> mOverlayPointRectF;
    private List<Bitmap> mOverlayBitmaps;
    ...
    public List<RectF> getOverlayPointRectF(){
        return mOverlayPointRectF;
    }

    public void setOverlayPointRectF(List<RectF> rectfs){
        mOverlayPointRectF=rectfs;
    }
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if(mOverlayBitmaps==null||mOverlayBitmaps.isEmpty()){
            return;
        }
        if(mOverlayPoint==null||mOverlayPoint.isEmpty()){
            return;
        }
        if(mOverlayPointRectF==null){
            mOverlayPointRectF=new ArrayList<RectF>();
        }
        mOverlayPointRectF.clear();
        ...
        float top = f[5];
        float scale = f[0];
        float right = left + rect.width() * scale;
        float bottom = top + rect.height() * scale;

        Paint print=new Paint();

        for(int i=0;i<mOverlayPoint.size();i++){
            PointF pointf=mOverlayPoint.get(i);
            float overlayWOffset= mOverlayBitmaps.get(i).getWidth()/2;
            if(pointf.x>rect.width()||pointf.y>rect.height()){
                mOverlayPointRectF.add(new RectF(0,0,0,0));
            }else{
                float x=pointf.x*scale+left;
                float y=pointf.y*scale+top;
                if(x<0||x>getWidth()||y<0||y>getHeight()){
                    mOverlayPointRectF.add(new RectF(0,0,0,0));
                }else{
                    RectF dst=new RectF(x-overlayWOffset,y,x+overlayWOffset,y+mOverlayBitmaps.get(i).getHeight());
                    canvas.drawBitmap(mOverlayBitmaps.get(i),null,dst,print);
                    mOverlayPointRectF.add(dst);
                }
            }
        }

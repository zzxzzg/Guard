    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (!mScaleDetector.isInProgress()) {
            ...
            if(action==MotionEvent.ACTION_DOWN) {
                isMoveOverlay = false;
                List<RectF> rectfs = getOverlayPointRectF();
                downX = event.getX();
                downY = event.getY();
                if (rectfs != null && !rectfs.isEmpty()) {
                    for (int i = 0; i < rectfs.size(); i++) {
                        RectF dst = rectfs.get(i);
                        if (dst.contains(downX, downY)) {
                            isMoveOverlay = true;
                            overlayIndex = i;
                            break;
                        }
                ...
                if(isMoveOverlay){
                    ...
                    float moveX=(event.getX()-downX)/scale/getBaseScale();
                    float moveY=(event.getY()-downY)/scale/getBaseScale();
                    downX=event.getX();
                    downY=event.getY();
                    PointF p=getOverlayPoint().get(overlayIndex);
                    p.set(p.x+moveX,p.y+moveY);
                    invalidate();
                }
            }
